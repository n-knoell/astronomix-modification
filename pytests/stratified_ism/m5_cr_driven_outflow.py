...
def _girichidis_fig1_style_plot(state, registered_variables, mid_index, t_yr, out_path):
    """Girichidis et al. (2016) Fig. 1-style structure plot: edge-on density
    (x-z slice through the box center), face-on density (x-y slice at the
    midplane), and CR energy density in the midplane (x-y slice at the
    midplane). One column, not the paper's three (we only have one run: the
    combined thermal+CR-feedback case, their rightmost panel), since M6
    (MHD/anisotropic transport) is the only planned follow-up that would add
    a second physically distinct configuration to compare against.
    """
    rho = np.asarray(state[registered_variables.density_index])
    e_cr = np.asarray(state[registered_variables.cosmic_ray_e_index])

    if not (np.any(rho > 0) and np.any(e_cr > 0)):
    ...
    im0 = ax0.imshow(
        edge_on_density, origin="lower", extent=[0, L_XY, 0, L_Z],
        norm=LogNorm(vmin=max(edge_on_density.min(), 1e-8), vmax=edge_on_density.max()),
        cmap="magma", aspect="auto",
    )
    ax0.axhline(Z0, color="cyan", ls=":", lw=1)
    ax0.set_xlabel("x [pc]")
    ax0.set_ylabel("z [pc]")
    ax0.set_title(f"Density, edge-on (y={L_XY / 2:.1f} pc)")
    fig.colorbar(im0, ax=ax0, label="density [code]")

    im1 = ax1.imshow(
        face_on_density, origin="lower", extent=[0, L_XY, 0, L_XY],
        norm=LogNorm(vmin=max(face_on_density.min(), 1e-8), vmax=face_on_density.max()),
        cmap="magma", aspect="equal",
    )
    ax1.set_xlabel("x [pc]")
    ax1.set_ylabel("y [pc]")
    ax1.set_title("Density, face-on (midplane)")
    fig.colorbar(im1, ax=ax1, label="density [code]")

    e_cr_floor = max(float(face_on_e_cr.max()) * 1e-8, 1e-30)
    im2 = ax2.imshow(
        face_on_e_cr, origin="lower", extent=[0, L_XY, 0, L_XY],
    ...
    fig.colorbar(im2, ax=ax2, label=r"$e_{cr}$ [code]")

    fig.suptitle(f"M5 structure at t={t_yr:.2e} yr (Girichidis et al. 2016 Fig. 1 style)")
    fig.tight_layout()
